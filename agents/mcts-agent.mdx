...
<ResponseField name="value" type="float">
  The win/visit ratio (`res.win / res.N`) of the selected child node. Reflects
  the tree's confidence in the chosen move.
</ResponseField>
...
<ParamField path="depth" type="int">
  Remaining depth to build. The initial call uses `4`; recursion stops when
  `depth <= 0`.
</ParamField>
...
<Note>
  If `leaf` has no valid moves (terminal state) **or** if `leaf.N == 0`
  (unvisited leaf), expansion returns `(leaf, depth)` unchanged without
  incrementing depth. The early-return condition in source is:
...
<ParamField path="result" type="str">
  The string returned by `simulation()`. The score added to each node's `win`
  tally is: `+1` for `"win"`, `-1` for `"loss"`, `0` for `"draw"`.
</ParamField>

<ParamField path="child" type="Node">
  The simulated child node. Its ancestors are stored in `self.parents` and are
  all updated in the loop.
</ParamField>
...
<Warning>
  The `win` parameter is not used inside `update()`. The conditional checks
  `self.win == True` (the instance field), so `self.win` only increments when
  it is already `1`. This is a quirk of the source implementation; direct field
  mutation via `back_propagation()` is the mechanism actually used during
  gameplay.
</Warning>
