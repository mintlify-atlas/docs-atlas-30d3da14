...
|---|---|---|
| `q_values` | `dict` | The Q-table. Keys are state-action strings (board cells concatenated with the column index); values are `float` Q-scores. Populated lazily — unseen (state, action) pairs default to `0`. |
| `total_rewards` | `float` | Accumulated reward for the current episode. Reset externally between episodes. |
| `game_status` | `str` | Game outcome flag read by `take_action()` on its terminal-update call. Set this to `"running"` (default), `"win"`, `"loss"`, or `"draw"` by the game loop before the final `take_action()` call. |
...
When the game loop sets `game_status` to `"win"`, `"loss"`, or `"draw"` and calls `take_action()` one final time, the method updates the Q-value for the last move with the terminal reward and returns:
...
### `epislon_greedy_policy(next_q) -> int`

Selects a column using the epsilon-greedy strategy.
...
**Returns:** `int` — the chosen column index.
...
Checks four directions (diagonal, anti-diagonal, horizontal, vertical) from the cell where the last piece landed. Returns `True` if `self.player`'s token appears four times consecutively.
...
for episode in range(50000):
    ...
    while True:
        ...
        if end:
            ...
            player2.take_action()   # terminal Q-value update
            break
```
