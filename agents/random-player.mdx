...
**Returns:** `int` (specifically `numpy.int64`) — the chosen column index.

```python
# Internally uses numpy's random.choices with a uniform distribution
...
**Returns:** `bool` — `True` if four consecutive tokens of `self.player` exist through the placed cell.
